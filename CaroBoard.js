    ...
    drawTable() {
        ...
        for (let i = 0; i < this.size; i++) {
            ...
            for (let j = 0; j < this.size; j++) {
                data += '<td class = "col"> <div class="cell" onClick="play(id)" onMouseOver="MouseOver(id)" onMouseOut="MouseOut(id)" id="' + (j + i * this.size) + '" value="-1"></div></td>'
            }
            data += '</tr>'
        }
        data += '</table>'
...
function play(id) {
    ...
    if (currentP === 1) {
        document.getElementById("imgPlayer").style.backgroundImage = "url('img/Xpng.png')"
    } else if (currentP === 0) {
        document.getElementById("imgPlayer").style.backgroundImage = "url('img/Opng.png')"
    } else {
...
function WinGame() {
    ...
    for (let i = 0; i < size; i++) {
        for (let j = 0; j < size; j++) {
            if (winHor(i, j, board) || winVer(i, j, board) || winCross1(i, j, board) || winCross2(i, j, board)) {
                for (let k = 0; k < l_win.length; k++) {
                    document.getElementsByClassName("cell").item(l_win[k]).style.backgroundColor = "yellow";
                    result = true;
                }
            }
        }
...
function winCross1(i, j, board) {
    ...
    for (let k = 0; k <= 4; k++) {
        if (board[(j + k) + (i + k) * size] === player && board[(j + k) + (i + k) * size] !== -1) {
            count++;
...
function MouseOut(id) {
    ...
    document.getElementsByClassName("cell").item(id).style.backgroundColor = "#FFF";
}
